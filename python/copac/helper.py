...
def emulatePhotoz(z,mag,magerr):
    import joblib
    from sklearn.ensemble import RandomForestRegressor

    loaded_model = joblib.load(open('emuPhotoZ_bpz_dnf_random_forest.joblib', 'rb'))
    bpz_model_mean, bpz_model_error, dnf_model_mean, dnf_model_error = loaded_model

    X  = [mi for mi in mag]
    X += [mi_err for mi_err in magerr]
    X += [(mag[:,0]-mag[:,1]),(mag[:,1]-mag[:,2]),(mag[:,2]-mag[:,3])]
    X = np.array(X).T

    z_noise = bpz_model_mean.predict(X)
    zerr = bpz_model_error.predict(X)
...
def do_galaxy_cuts(galaxy,clusters,zrange=[0.,1.4]):
    ## do some cuts
...
def _readCosmoDC2(galaxyInFile, clusters, h=0.7, rmax=3,r_in=8,r_out=10, radius=12, window=0.1, zrange=(0.01,1.),Nflag=0,colNames=None,simulation=False,emulator=True):
    ...
    ## emulator Photoz
    if emulator:
        ## input: z (true_redshift), magnitudes, magnitudes errors
        z_noise, zerr = emulatePhotoz(z,mag,magerr)
